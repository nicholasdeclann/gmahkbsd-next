...
async function fetchScheduleData(columnOffset: number) {
  const [laguSionText, kertasAcaraText, sheetText] = await Promise.all([
  ...
  kertasAcaraJson.table.rows.forEach((row: any) => {
    ...
    if (label === "lagu buka") {
      laguBukaNum = row.c[2]?.v;
      laguBukaSSNum = row.c[9]?.v;
    } else if (label === "lagu tutup") {
      laguTutupNum = row.c[2]?.v;
      laguTutupSSNum = row.c[9]?.v;
    } else if (label === "judul khotbah") {
      judulKhotbah = row.c[2]?.v;
    } else if (label === "ayat inti") {
      ayatInti = row.c[2]?.v;
    } else if (label === "ayat bersahutan") {
      ayatBersahutanText = row.c[2]?.v;
    }
  });

  const json = JSON.parse(sheetText.substring(47, sheetText.length - 2));
  const cachedRows = json.table.rows;
  ...
  if (columnIndex === -1) {
    return {
      scheduleDate: `Jadwal tidak ditemukan untuk ${targetDate}`,
      ssData: {},
      khotbahData: {},
      pelayananData: [],
      diakoniaData: [],
  ...
  const scheduleDate = `Sabat, ${targetDate}`;
  const ssData: any = {};
  const khotbahData: any = {};
  const pelayananData: any[] = [];
  const diakoniaData: any[] = [];
  ...
  for (let i = 4; i < cachedRows.length; i++) {
    ...
    if (currentSection === "ssDewasa") {
      ssData[role.toLowerCase()] = item;
    } else if (currentSection === "khotbah") {
      khotbahData[role.toLowerCase()] = item;
    } else if (currentSection === "pelayanan") {
      pelayananData.push(item);
    } else if (currentSection === "diakonia") {
      diakoniaData.push(item);
    }
  }
  ...
  const lastUpdated = `Data partisipan diambil pada: ${timeString}`;
...
export default async function KertasAcara({
  searchParams,
}: {
  searchParams: Promise<{ week?: string }>;
}) {
  const params = await searchParams;
  const columnOffset = params.week === "next" ? 1 : 0;
  ...
  const {
    ...
    ayatBersahutanText,
    lastUpdated,
  } = await fetchScheduleData(columnOffset);
